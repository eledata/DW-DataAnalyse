create or replace PROCEDURE               INV_SAP_DQD_PO_IMPORT IS
...
DBMS_OUTPUT.PUT_LINE(substr(STMT_QUERY,201,200)  ); --CPZ added to limit exception 20000 line too long
                ...
                commit;
                select max(VALUE_NUMBER) into RECORD_COUNT9  from AAA_PROCEDURE_VALUE_EXCHANGE where PROCEDURE_RECORD_ID = REC_ID ;
                commit;
                select sum(mengescheduledqty)  into INSERT_COUNT1  from dwq$librarian.INV_SAP_PO_EKET  ;
...
DBMS_OUTPUT.PUT_LINE(substr( STEP_NAME,1,254) );
		     --- check with Dual if the DB connetion is live... 
			   ...
			   select max(VALUE_NUMBER) into RECORD_COUNT  from AAA_PROCEDURE_VALUE_EXCHANGE where PROCEDURE_RECORD_ID = REC_ID ;
...
DBMS_OUTPUT.PUT_LINE( '' || STEP_NAME );
	...
	-----------------------------------------------------------		
          ...
          begin 
            ...
            --select count(9) into HOLDER1   from SAPECC_DLY_LIBRARIAN.PLAF@DQD.mke.ra.rockwell.com;  --CPZ 02-01-2012
             ...
             select count(9)  into RECORD_COUNT10
		  ...
		  when others then
             ...
             begin
                ...
                Aaa_Qdw_Script_Exec_Immediate ('truncate table INV_SAP_LIKP_LIPS_DAILY');
                 ...
                 insert /* + append */ into INV_SAP_LIKP_LIPS_DAILY select * from INV_SAP_LIKP_LIPS_DAILY_V nologging; --CPZ 01-20-2012 needs nologging
    ...
    begin            
     ...
     begin            
		...
		IF STATUS = 'IP' THEN
	      STATUS := 'P';
		     AAA_QDW_MATERIALS_SCRIPT_EMAIL ( REC_ID,SCRT_GRP || ' STATUS SUCCSESSFUL COMPLETION '  ||STATUS ,'Successful completion '||CHR(13)
			 ||'Stored Proc '||SCRT_GRP||'@QDWD ' || CHR(13)||ORACLE_ERROR);
		END IF;
...
EXCEPTION
 	            ...
 	            DBMS_OUTPUT.PUT_LINE(substr(ORACLE_ERROR,1,254)) ;
                    commit;    
	 	   AAA_QDW_MATERIALS_SCRIPT_EMAIL ( REC_ID,SCRT_GRP || ' STATUS '  ||STATUS ,'Automatic Notification Error '||CHR(13)
			 ||'Stored Proc '||SCRT_GRP||'@QDWD ' || CHR(13)||ORACLE_ERROR);
		  when table_Missing then 
				      RECORD_COUNT:=0;
				       ORACLE_ERROR :=  SUBSTR(DBMS_UTILITY.FORMAT_ERROR_STACK,1,1000) || SUBSTR(DBMS_UTILITY.FORMAT_CALL_STACK,1,500);
