...
SELECT * FROM VIEW_INV_SAP_SPIKEPROD;
1.
SELECT ID,
  MATERIAL,
  CATALOG#,
  ...
  SHIPPING_POINT,
  ULTIMATE_SOURCE
  FROM VIEW_INV_SAP_SALES_HST_LC WHERE SALES_DOC IN ('6000759500',
'6000754887');
...
SELECT * FROM INV_SAP_PP_OPT_X WHERE ID = 'PN-D167002_5040' 
...
SELECT MATERIALID
        ...
        FROM INV_SAP_IO_INPUTS_DAILY
        WHERE INPUT_TYPE = 'PLAN_PD_PO'
        AND DATEDELIVERY < TO_CHAR(sysdate + 37)
        AND PLANTID IN ('5040',
'5050',
'5070',
'5100',
...
AND SLFDTSTATDELIVERYDATE      < TO_CHAR(sysdate + 37)
AND PLANTID IN ('5040',
'5050',
'5070',
'5100',
...
SELECT MATERIALID, PLANTID,ULTIMATE_SOURCE FROM INV_SAP_PP_OPTIMIZATION WHERE PLANTID IN ('1090','1180') AND 
MATERIALID IN (
'PN-D13058',
'PN-142623',
'PN-65394',
...
SELECT MATERIALID
        ...
        FROM INV_SAP_PP_PO_HISTORY
        WHERE BSART_PURCHDOCTYPE      IN ('ZST','ZNB')
        AND ELIKZDELIVERYCOMPLETEDIND IS NULL AND PLANTID = ('5040')
        AND SLFDTSTATDELIVERYDATE             < TO_CHAR(sysdate + 41);
        ...
        PO_OPENQTY AS OPEN_PLAN_QTY
      FROM INV_SAP_IO_INPUTS_DAILY
      WHERE INPUT_TYPE = 'PLAN_PD_PO' AND PLANTID = ('5040')
      AND DATEDELIVERY < TO_CHAR(sysdate + 41);
...
SELECT MATERIALID
        ...
        FROM INV_SAP_PP_PO_HISTORY
        WHERE BSART_PURCHDOCTYPE      IN ('ZST','ZNB')
        AND ELIKZDELIVERYCOMPLETEDIND IS NULL AND PLANTID = ('5200')
        AND SLFDTSTATDELIVERYDATE             < TO_CHAR(sysdate + 26);
        ...
        PO_OPENQTY AS OPEN_PLAN_QTY
      FROM INV_SAP_IO_INPUTS_DAILY
      WHERE INPUT_TYPE = 'PLAN_PD_PO' AND PLANTID = ('5200')
      AND DATEDELIVERY < TO_CHAR(sysdate + 26);
...
SELECT * FROM 
(
SELECT ID AS LC_ID,VENDOR_KEY,VENDOR_ITEM,ULTIMATE_SOURCE
                FROM INV_SAP_PP_OPT_X
  WHERE PLANT = '5040' and MATERIAL in ('100-C09EJ01 A',
'100-C09EJ10 A',
'100-C09KF10 A',
'100-C09KJ10 A',
'100-C12EJ10 A',
...
LEFT JOIN
(SELECT ID,  MRP_CONTROLLER,
      MRP_CONTROLLER_KEY FROM INV_SAP_PP_OPT_X
)V_D
ON LC.VENDOR_ITEM = V_D.ID;

select * from VIEW_INV_SAP_STO_GRCOPEN where PROD_HIERARCHY_18 = 'PLSA0893A8914A4431';

SELECT MATERIALID, CHANGED_ON_DATE, ACT_GI_DATE,PLANTID FROM INV_SAP_LIKP_LIPS_DAILY where PLANTID in ('1090','1180');

select count(*) from(
SELECT distinct MATERIAL,PROD_BU FROM INV_SAP_PP_OPT_X WHERE  PROD_BU = 'SDB');

select * from 
(SELECT distinct MATERIAL,CATALOG_DASH,PROD_BU FROM INV_SAP_PP_OPT_X WHERE  PROD_BU = 'SDB' and plant = '5040')matl
left join
(SELECT EBELNPURCHDOCNO,MATERIALID,PLANTID,BUDATPOSTINGDATE,MENGESCHEDULEDQTY FROM INV_SAP_PP_PO_HISTORY where PLANTID = '5040' and ETENRPURCHDELIVSCHLINESA
 = '1')issue
on matl.material = issue.materialid;

select * from INV_SAP_PP_PO_HISTORY where   EBELNPURCHDOCNO= '6301911091'
