...
1. Oracle Client Setup
	...
	1.3 Ini Setup
		...
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PP_OPTIMIZATION@ROCKWELL_DW_DBLINK WHERE PLANTID = '5040';
    ...
    SELECT * FROM INV_SAP_PLAN_PARAM_USERLOG;
		...
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_SALES_VBAK_VBAP_VBUP@ROCKWELL_DW_DBLINK WHERE SALESDOC = '6502731313';
		...
		DROP TABLE INV_SAP_PP_FRCST_PBIM_1090;
		CREATE TABLE INV_SAP_PP_FRCST_PBIM_1090 AS
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PP_FRCST_PBIM_PBED@ROCKWELL_DW_DBLINK WHERE PLANTID IN ('1090');

		DROP TABLE INV_SAP_PP_FRCST_PBIM_ALL;
		CREATE TABLE INV_SAP_PP_FRCST_PBIM_ALL AS
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PP_FRCST_PBIM_PBED@ROCKWELL_DW_DBLINK;
    
    SELECT * FROM INV_SAP_PP_FRCST_PBIM_1090;
    ...
    CREATE TABLE INV_SAP_PP_PO_HISTORY_ALL AS SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PP_PO_HISTORY@ROCKWELL_DW_DBLINK;
		...
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_SALES_HST@ROCKWELL_DW_DBLINK WHERE Plantid = '5040';
    
		DROP TABLE INV_SAP_PP_MVKE;
		CREATE TABLE INV_SAP_PP_MVKE AS
		SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PP_MVKE@ROCKWELL_DW_DBLINK;

SELECT * FROM INV_SAP_PP_MVKE WHERE MATERIALID = '871A-TS4-N3 B' 
SELECT * FROM INV_SAP_PP_PO_HISTORY_ALL where ebelnpurchdocno = '6302869401';

SELECT LOT_ROUNDING_VALUE FROM INV_SAP_PP_OPTIMIZATION where MATERIALID = '700-HLT1U2 A'



SELECT * FROM INV_SAP_PP_PO_HISTORY_ALL WHERE 
...
--Daily Jobs
    ...
    CREATE TABLE INV_SAP_PP_OPTIMIZATION_STABLE AS SELECT * FROM INV_SAP_PP_OPTIMIZATION;
    ...
    SELECT distinct MATERIAL,CATALOG_DASH,MAT_DESC,LOT_SIZE_QTY FROM INV_SAP_PP_OPT_X
    ...
    SELECT COUNT(*) FROM
    (SELECT DISTINCT PLANT, MATERIAL, CATALOG_DASH, CATALOG_NO_DASH, ULTIMATE_SOURCE FROM INV_SAP_PP_OPT_X WHERE  PLANT IN  ('5040', '5050', '5100', '5110', '5120', '5160', '5190', '5200','5070','5140'));
    ...
    Job running time: 5mins
		...
		Job Initialization: 
    SELECT * FROM INV_SAP_FC00_STATS;
...
END;   
		...
		Job Initialization: 
      SELECT DISTINCT STOCK_IN_TRANSIT_QTY FROM INV_SAP_INV_SUM_STATS where snapshot_date between to_char(sysdate - 1) and to_char(sysdate);
...
select * from INV_SAP_SALES_HST WHERE PLANTID IN ('5050', '5110') AND LASTACTGIDATE BETWEEN TO_CHAR(SYSDATE - 390) AND TO_CHAR(SYSDATE + 1);
  ...
  3.3 Job Name: Job_Weekly_INV_REC
    ...
    FROM
      (SELECT 
        SYSDATE-1||''_''||MATERIALID||''_''||PLANTID AS INV_ID,
        MATERIALID||''_''||PLANTID AS ID,
        SYSDATE-1 AS REC_DATE,
        MATERIALID,
        PLANTID,
        OH_QTY
  ...
  SELECT * FROM INV_SAP_INV_REC WHERE  REC_DATE BETWEEN TO_CHAR(SYSDATE - 1) AND TO_CHAR(SYSDATE +3)
  ...
  SELECT * FROM INV_SAP_INVENTORY_BY_PLANT WHERE PLANTID = '5100' AND MATERIALID = '440F-A1302S A'
    ...
    SELECT * FROM INV_SAP_INV_REC WHERE ID = '1756-IB16 A_5040'
...
SELECT count(*) FROM INV_SAP_MATERIAL_CATALOG where CATALOG_STRING1 = 'PN-141666';
  ...
  3.3 Job Name: Job_Daily_Inv_PP_X
    ...
    BEGIN
      INSERT_TABLE_INV_OPT := 'INSERT INTO INV_PPXST_ND_2015
