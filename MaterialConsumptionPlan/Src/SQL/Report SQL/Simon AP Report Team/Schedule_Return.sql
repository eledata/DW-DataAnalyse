...
----V2----
DROP VIEW VIEW_INV_SAP_SCH_RE_SEL;
CREATE VIEW VIEW_INV_SAP_SCH_RE_SEL AS 
 SELECT RE_BSVSTGS.ID            AS ID,
    RE_BSVSTGS.CUST_RE_PLANT      AS CUST_RE_PLANT,
    RE_BSVSTGS.CUST_RE_MATL       AS CUST_RE_MATL,
    RE_BSVSTGS.CUST_RE_QTY        AS CUST_RE_QTY,
            ...
            FROM
              ...
              LEFT JOIN
                (SELECT ID,
                  ...
                  VENDOR_ITEM,
                  ULTIMATE_SOURCE
                FROM INV_SAP_PP_OPT_X WHERE CATALOG_DASH = '2711P-B10C4A8' AND PLANT = '5200'
                )RETURN_PLANT
              ON RETURN_PLANT.TMP_ID = SCHED_RE.TMP_ID
              )RE_BS
            LEFT JOIN
...
----V3----
DROP VIEW VIEW_INV_SAP_SCH_RE_SEL_V3;
CREATE VIEW VIEW_INV_SAP_SCH_RE_SEL_V3 AS 
SELECT RE_BV2.ID            AS ID,
  RE_BV2.CUST_RE_PLANT      AS CUST_RE_PLANT,
  RE_BV2.CUST_RE_MATL       AS CUST_RE_MATL,
  RE_BV2.CUST_RE_QTY        AS CUST_RE_QTY,
  RE_BV2.CUST_RE_SER        AS CUST_RE_SER,
  RE_BV2.CUST_RE_PRICE      AS CUST_RE_PRICE,
  RE_BV2.CUST_RE_AMOUNT     AS CUST_RE_AMOUNT,
  RE_BV2.CUST_REMARK        AS CUST_REMARK,
  RE_BV2.RE_MATERIAL        AS RE_MATERIAL,
  RE_BV2.RE_PLANT           AS RE_PLANT,
  RE_BV2.RE_D_CHAIN_BLK     AS RE_D_CHAIN_BLK,
  RE_BV2.RE_UNIT_COST       AS RE_UNIT_COST,
  RE_BV2.RE_DELIVERY_UNIT   AS RE_DELIVERY_UNIT,
  RE_BV2.RE_STRATEGY_GRP    AS RE_STRATEGY_GRP,
  RE_BV2.RE_OH_QTY          AS RE_OH_QTY,
  RE_BV2.RE_VENDOR          AS RE_VENDOR,
  RE_BV2.RE_MATL_TYPE       AS RE_MATL_TYPE,
  RE_BV2.RE_VENDOR_ITEM     AS RE_VENDOR_ITEM,
  RE_BV2.RE_ULTIMATE_SOURCE AS RE_ULTIMATE_SOURCE,
  RE_BV2.VP_STRATEGY_GRP    AS VP_STRATEGY_GRP,
  RE_BV2.VP_OH_QTY          AS VP_OH_QTY,
  RE_BV2.VP_Q4_SUM          AS VP_Q4_SUM,
  RE_BV2.Q1_SHIP_QTY        AS Q1_SHIP_QTY,
  RE_BV2.Q2_SHIP_QTY        AS Q2_SHIP_QTY,
  RE_BV2.Q3_SHIP_QTY        AS Q3_SHIP_QTY,
  RE_BV2.Q4_SHIP_QTY        AS Q4_SHIP_QTY,
  RE_BV2.Q1_LN_QTY          AS Q1_LN_QTY,
  RE_BV2.Q2_LN_QTY          AS Q2_LN_QTY,
  RE_BV2.Q3_LN_QTY          AS Q3_LN_QTY,
  RE_BV2.Q4_LN_QTY          AS Q4_LN_QTY,
  RE_BV2.ALLOW_RETURN_FLG   AS ALLOW_RETURN_FLG,
  RE_BV2.SG_CHECK           AS SG_CHECK,
  RE_BV2.SERISES            AS SERISES,
  RE_BV2.EX_TO_USD          AS EX_TO_USD,
  DSTATS.MAX_DIS_NAME       AS MAX_DIS_NAME,
  DSTATS.MAX_DIS_USAGE_QTY  AS MAX_DIS_USAGE_QTY,
  DSTATS.TOT_USAGE_QTY      AS TOT_USAGE_QTY,
  DSTATS.PESTG              AS PESTG,
  DSTATS.DISTRIBUTOR_COUNT  AS DISTRIBUTOR_COUNT
FROM
  (SELECT RE_BSVSTGS.ID           AS ID,
    RE_BSVSTGS.CUST_RE_PLANT      AS CUST_RE_PLANT,
    RE_BSVSTGS.CUST_RE_MATL       AS CUST_RE_MATL,
...
LEFT JOIN
  (SELECT ID,
    ...
    MAX_DIS_USAGE_QTY,
    TOT_USAGE_QTY,
    ROUND(PERSENTAGE,2) AS PESTG,
    DISTRIBUTOR_COUNT
  FROM INV_SAP_DSTR_STATS 
  )DSTATS
ON DSTATS.ID = RE_BV2.ID;
  
--Generate the Schedule Report
SELECT DISTINCT CUST_RE_PLANT,
  CUST_RE_MATL,
  CUST_RE_QTY,
    CASE
    WHEN CUST_RE_SER = '#'
  ...
  RE_ULTIMATE_SOURCE,
  EX_TO_USD,
  NVL(CUST_RE_QTY,0)*NVL(CUST_RE_PRICE,0)*NVL(EX_TO_USD,0)
  AS INV_VALUE
FROM VIEW_INV_SAP_SCH_RE_SEL;
...
SELECT DISTINCT CUST_RE_PLANT,
  ...
  NVL(CUST_RE_QTY,0)*NVL(CUST_RE_PRICE,0)*NVL(EX_TO_USD,0)
  AS INV_VALUE
FROM VIEW_INV_SAP_SCH_RE_SEL_V3;
------------------------------------------------------------------------------------------------------------------------
--BK
SELECT DISTINCT CUST_RE_PLANT,
  CUST_RE_MATL,
...
----Current serial
select distinct materialid from INV_SAP_SALES_HST WHERE PLANTID IN ('5040', '5050', '5100', '5110', '5120', '5160', '5190', '5200','5070','5140');
select * from INV_SAP_DSTR_STATS;
SELECT * FROM VIEW_INV_SAP_SCH_RE_SELMD;
SELECT DISTINCT 
  ID,
  CUST_RE_PLANT,
  CUST_RE_MATL,
...
SELECT DISTINCT 
  ...
  NVL(CUST_RE_QTY,0)*NVL(CUST_RE_PRICE,0)*NVL(EX_TO_USD,0)
  AS INV_VALUE
FROM VIEW_INV_SAP_SCH_REMATL;

SELECT ID,
  MATERIAL,
  CATALOG,
  SOLD_TO_NAME,
  DISTRIBUTOR_USAGE_QTY,
  Q1_DIS_USAGE_QTY,
  Q2_DIS_USAGE_QTY,
  Q3_DIS_USAGE_QTY,
  Q4_DIS_USAGE_QTY
FROM INV_SAP_DSTR_QSTATSLIST WHERE ID = 'ID_1';

SELECT COUNT(*) FROM INV_SAP_DSTR_STATS;
SELECT ID,
    MAX_DIS_USAGE_QTY,
    MAX_DIS_NAME,
    TOT_USAGE_QTY,
...
FROM
  (SELECT ID,
    ...
    DISTRIBUTOR_COUNT,
    ROWID
  FROM INV_SAP_DSTR_STATS WHERE (ID,ROWID) IN (SELECT ID,MIN(ROWID) FROM INV_SAP_DSTR_STATS GROUP BY ID)) WHERE ID = 'PN-22073_5200'
  
 SELECT * FROM INV_SAP_DSTR_STATS WHERE ID = 'PN-22073_5200';
