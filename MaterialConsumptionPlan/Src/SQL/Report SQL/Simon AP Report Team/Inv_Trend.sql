...
SELECT * FROM INV_SAP_INV_REC WHERE REC_DATE > SYSDATE - 61;
...
SELECT * FROM DWQ$LIBRARIAN.INV_SAP_PLAN_PARAM_USERLOG@ROCKWELL_DW_DBLINK where LOG_DATE between sysdate - 1 and sysdate;
INSERT
INTO DWQ$LIBRARIAN.INV_SAP_PLAN_PARAM_USERLOG@ROCKWELL_DW_DBLINK
  (
    USERID,
    COMPUTERID,
    APPLICATION ,
...
SELECT * FROM INV_SAP_INV_REC WHERE REC_DATE > SYSDATE - 1;
select sum(oh_val) FROM INV_SAP_INV_REC WHERE REC_DATE > SYSDATE - 1;

select * From VIEW_INV_SAP_INV_TREND WHERE REC_DATE > SYSDATE - 1;
...
SELECT * FROM VIEW_INV_SAP_INV_TREND WHERE KEY <> 0 AND PLANT IN ('5100') AND MATERIAL = '440F-A1302S A'
...
SELECT LAST_REVIEW_DATE
    ...
    MATERIAL,
    STATUS
  FROM STOCK_ITEM_STATUS_BY_ITEM where LAST_REVIEW_DATE  between SYSDATE - 1 and sysdate;
...
FROM
  (SELECT MATERIALID
    ...
    LOCATIONID,
    OH_QTY
  FROM INV_SAP_INVENTORY_BY_PLANT_SLC
  WHERE PLANTID IN ('5040', '5050', '5100', '5110', '5120', '5160', '5190', '5200','5070','5140','1090','1180','4000')
  )INV_SLC
LEFT JOIN
  (SELECT ID,
    CATALOG_DASH,
