...
CREATE TABLE STOCK_ITEM_PERFORMANCE AS
  ...
  PP_FC_AVG.MIN_INV AS MIN_INV,
  PP_FC_AVG.TARGET_INV AS TARGET_INV,
  PP_FC_AVG.MAX_INV AS MAX_INV,
  PP_FC_AVG.MIN_INV_VAL AS MIN_INV_VAL,
  PP_FC_AVG.TARGET_INV_VAL AS TARGET_INV_VAL,
  PP_FC_AVG.MAX_INV_VAL AS MAX_INV_VAL,
  PP_FC_AVG.OVER_MAX_INV_VAL AS OVER_MAX_INV_VAL,
...
FROM
  ...
  FROM
    (SELECT ID,
      ...
      TARGET_INV,
      MAX_INV,
      NVL(MIN_INV*UNIT_COST,0) AS MIN_INV_VAL,
      NVL(TARGET_INV*UNIT_COST,0) AS TARGET_INV_VAL,
      NVL(MAX_INV*UNIT_COST,0) AS MAX_INV_VAL,
      NVL(OH_QTY*UNIT_COST-MAX_INV*UNIT_COST,0) AS OVER_MAX_INV_VAL
...
5.Fetch Data STOCK_ITEM_STATUS_BY_BU_WEEK	
  DELETE SELECT * FROM STOCK_ITEM_STATUS_BY_BU_WEEK WHERE LAST_REVIEW_DATE BETWEEN TO_CHAR(SYSDATE- 4) AND TO_CHAR(SYSDATE + 1)
...
SELECT * FROM STOCK_ITEM_STATUS_BY_ITEM WHERE LAST_REVIEW_DATE > sysdate -2
  
CREATE TABLE STOCK_ITEM_STATUS_BY_BU_WEEK_B AS 
SELECT * FROM STOCK_ITEM_STATUS_BY_BU_WEEK WHERE LAST_REVIEW_DATE > sysdate - 2

  STOCK_ITEM_STATUS_BY_BU_WEEK
  
  
select * FROM STOCK_ITEM_STATUS_BY_BU_WEEK WHERE LAST_REVIEW_DATE BETWEEN TO_CHAR(sysdate - 9) AND TO_CHAR(sysdate - 2); 

DELETE FROM STOCK_ITEM_STATUS_BY_BU_WEEK WHERE LAST_REVIEW_DATE > sysdate - 3;


SELECT * FROM STOCK_ITEM_STATUS_BY_ITEM WHERE LAST_REVIEW_DATE > sysdate - 3;


------------------------------------------------------------------------------------------------------------------------
--Create the new Chart for the Stock Item Metric
SELECT LAST_REVIEW_DATE,
  PLANT,
  BU,
  MATERIAL,
  ...
  MATL_TYPE,
  STATUS,
  ROUND(NVL(OH_VAL,0)) AS OH_VAL
FROM VIEW_INV_SAP_INV_REC_STATUS;

DROP VIEW VIEW_INV_SAP_INV_REC_STATUS;
CREATE VIEW VIEW_INV_SAP_INV_REC_STATUS AS
  SELECT
  SITBI.LAST_REVIEW_DATE,
  SITBI.PLANT,
  SITBI.BU,
  SITBI.MATERIAL,
  ISIR.STRATEGY_GRP,
...
FROM
  (SELECT LAST_REVIEW_DATE
    ...
    MATERIAL,
    STATUS
  FROM STOCK_ITEM_STATUS_BY_ITEM
  WHERE LAST_REVIEW_DATE > (SYSDATE - 100)
  )SITBI
LEFT JOIN
  (SELECT REC_DATE
    ||'_'
...
--FOR MPS FC CONSUMPTION...
DROP VIEW VIEW_STOCK_ITEM_PER_1090;
CREATE VIEW VIEW_STOCK_ITEM_PER_1090 AS
  SELECT SYSDATE                         AS LAST_REVIEW_DATE,
  PP_FC_AVG.ID                         AS ID,
  PP_FC_AVG.MATERIAL                   AS MATERIAL,
  PP_FC_AVG.CATALOG_DASH               AS CATALOG_DASH,
...
FROM
  ...
  FROM
    ...
    AND MATL_TYPE     IN ('ZFG','ZTG')
    AND PLANT         IN ('1090')
    AND MRP_CONTROLLER_KEY IN ('007','015')
    )PP
  LEFT JOIN
    (SELECT MATERIALID
      ||'_'
      ...
      PLANTID                             AS PLANTID,
      CEIL(SUM(PLNMG_PLANNEDQUANTITY)/18) AS FC_AVG_WEEK
    FROM INV_SAP_PP_FRCST_PBIM_1090
    WHERE (PDATU_DELIV_ORDFINISHDATE BETWEEN TO_CHAR(sysdate + 56) AND TO_CHAR(sysdate + 182))
    AND PLANTID                                                   IN ('1090')
    AND VERSBP_VERSION = '00'
    GROUP BY MATERIALID,
...
DROP VIEW VIEW_STOCK_ITEM_PER_AP;
CREATE VIEW VIEW_STOCK_ITEM_PER_AP AS
  SELECT SYSDATE                         AS LAST_REVIEW_DATE,
  PP_FC_AVG.ID                         AS ID,
  PP_FC_AVG.MATERIAL                   AS MATERIAL,
  PP_FC_AVG.CATALOG_DASH               AS CATALOG_DASH,
...
DROP VIEW VIEW_STOCK_ITEM_PER_APCDC;
CREATE VIEW VIEW_STOCK_ITEM_PER_APCDC AS
SELECT * FROM VIEW_STOCK_ITEM_PER_1090
UNION
SELECT * FROM VIEW_STOCK_ITEM_PER_AP;
