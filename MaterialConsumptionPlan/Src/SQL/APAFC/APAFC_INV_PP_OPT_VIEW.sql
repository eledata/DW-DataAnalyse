...
--    ---------   ---------------   ----------------------------------------------------
--    2015-1-27  Moyue           	INV PP OPT VIEW.
-----------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------
...
CREATE OR REPLACE FORCE VIEW VIEW_INV_SAP_PP_OPT_X AS
  ...
  ITEM_SOG_X.DELIVERY_UNIT          AS DELIVERY_UNIT,
...
FROM
  ...
  FROM
    (SELECT PP_BAS.ID,
      ...
      CEIL(PP_BAS.SAFETY_STOCK)                                                                                AS MIN_INV,
      CEIL(1.08*CEIL(PP_BAS.SAFETY_STOCK) + 0.5*CEIL(PP_BAS.LOT_SIZE_QTY))                                     AS TARGET_INV,
      PP_BAS.INV_MAX                                                                                           AS MAX_INV,
      PP_BAS.LOT_SIZE_QTY,
...
LEFT JOIN
  (SELECT ID,
    ...
    DIST_CHL,
    D_CHAIN_BLK,
    DELIVERY_UNIT, -- Add delivery unit
    CURRENT_SERIES
...
DROP TABLE INV_SAP_PP_OPT_BASIC;
CREATE TABLE INV_SAP_PP_OPT_BASIC AS SELECT * FROM VIEW_INV_SAP_PP_OPT_BASIC;


DROP VIEW VIEW_INV_SAP_PP_OPT_BASIC;
CREATE VIEW VIEW_INV_SAP_PP_OPT_BASIC AS 
SELECT PP_BAS.ID,
      PP_BAS.LAST_REVIEW,
      PP_BAS.MATERIAL,
      PP_BAS.PLANT,
      ...
      --Method changed in 09262014      
      CEIL(PP_BAS.SAFETY_STOCK)                                                                     AS MIN_INV,
      CEIL(1.08*CEIL(PP_BAS.SAFETY_STOCK) + 0.5*PP_BAS.LOT_SIZE_QTY)                                AS TARGET_INV,
      INV_MAX                                                                                       AS MAX_INV,
      PP_BAS.LOT_SIZE_QTY,
      PP_BAS.LOT_ROUNDING_VALUE,
      PP_BAS.LOT_SIZE_DISLS,
    ...
    FROM
      ...
      WHERE plant IN ('5040', '5050', '5100', '5110', '5120', '5160', '5190', '5200','5070','5140','1090');
      PP_BAS
    LEFT JOIN
      (SELECT MATERIALID
        ||'_'
