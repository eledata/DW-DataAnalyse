...
FROM
  ...
  FROM
    (SELECT REC_DATE
      ...
      MATL_TYPE,
      MRP_TYPE
    FROM INV_PPXST_MARAPR_2016 --Need to change when table vol is too big...
    )INV_OPT
  LEFT JOIN
...
select distinct REC_DATE from INV_SAP_INV_REC WHERE REC_DATE BETWEEN TO_CHAR(SYSDATE - 7) AND TO_CHAR(SYSDATE -6);
--Mon
SELECT distinct MATL_TYPE
...
SELECT * FROM INV_SAP_STOCK_OUT_REPSTORE WHERE UPLOADDATE BETWEEN TO_CHAR(SYSDATE - 3) AND TO_CHAR(SYSDATE +1)
...
FROM
  (SELECT TO_CHAR(UPLOADDATE,'iw') AS UPDATE_WK,
    ...
    THU,
    FRI
  FROM INV_SAP_STOCK_OUT_REPSTORE WHERE UPLOADDATE BETWEEN TO_CHAR(SYSDATE-36) AND TO_CHAR(SYSDATE - 8)
  )STOCKOUT
LEFT JOIN
