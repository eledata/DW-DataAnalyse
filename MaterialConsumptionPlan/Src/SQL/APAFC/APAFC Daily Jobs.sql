...
1. Oracle Client Setup
    ...
    SELECT * FROM DWQ$LIBRARIAN.INV_SAP_SALES_VBAK_VBAP_VBUP@ROCKWELL_DW_DBLINK where LINECREATEDATE between sysdate -3 and sysdate - 2 and plant = '5040'; 
...
END;
  ...
  SELECT DISTINCT MATERIALID
    ...
    WHERE MATERIALID = '140U-J-RVM21B C'
...
4.2 Job Name: Job_Monthly_INV_PP_CATA
    ...
    END;
		...
		DROP INDEX IDX_INV_SAP_R8_SALES_DC_MATL;
		CREATE INDEX IDX_INV_SAP_R8_SALES_DC_MATL ON INV_SAP_R8_SALES_DC_HST ("MATERIALID");
    CREATE INDEX IDX_INV_SAP_R8_SALES_DC_PLAT ON INV_SAP_R8_SALES_DC_HST ("PLANTID");
--------------------------------------------------------------------------------------------------------------------------------------------------------	
...
DECLARE
  CREATE_IDX_MATL      VARCHAR2(500);
  CREATE_IDX_PLAT      VARCHAR2(500);
  DROP_IDX_MATL        VARCHAR2(500);
  DROP_IDX_PLAT        VARCHAR2(500);
  TRUNCATE_R8_SALES_DC VARCHAR2(500);
  INSERT_R8_SALES_DC   VARCHAR2(1000);
BEGIN
  TRUNCATE_R8_SALES_DC:= 'TRUNCATE TABLE INV_SAP_R8_SALES_DC_HST';
  INSERT_R8_SALES_DC  := 'INSERT INTO INV_SAP_R8_SALES_DC_HST SELECT * FROM DWQ$LIBRARIAN.INV_SAP_R8_SALES_DC_HST@ROCKWELL_DW_DBLINK';
  CREATE_IDX_MATL     := 'CREATE INDEX IDX_INV_SAP_R8_SALES_DC_MATL ON INV_SAP_R8_SALES_DC_HST ("MATERIALID")';
  CREATE_IDX_PLAT     := 'CREATE INDEX IDX_INV_SAP_R8_SALES_DC_PLAT ON INV_SAP_R8_SALES_DC_HST ("PLANTID")';
  DROP_IDX_MATL       := 'DROP INDEX IDX_INV_SAP_R8_SALES_DC_MATL';
  DROP_IDX_PLAT       := 'DROP INDEX IDX_INV_SAP_R8_SALES_DC_PLAT';
  
  --Drop the index before update the table
  EXECUTE IMMEDIATE DROP_IDX_MATL;
  EXECUTE IMMEDIATE DROP_IDX_PLAT;
  --Update the table
  EXECUTE IMMEDIATE TRUNCATE_R8_SALES_DC;
  EXECUTE IMMEDIATE INSERT_R8_SALES_DC;
  COMMIT;
  --Create index on the table
  EXECUTE IMMEDIATE CREATE_IDX_MATL;
  EXECUTE IMMEDIATE CREATE_IDX_PLAT;
END;
